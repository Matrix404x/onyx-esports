...
import { LayoutDashboard, Trophy, Users, MessageSquare, Mic, LogOut, Menu, X, Link as LinkIcon, Activity, Star, Shield, Search } from 'lucide-react';
        ...
        <div className="flex h-screen bg-slate-950 text-white overflow-hidden">
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 border-r border-slate-800 transition-transform duration-300 md:relative md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                ...
                <nav className="p-4 space-y-2">
                    ...
                    <NavItem icon={<Search />} label="Find Players" onClick={() => navigate('/players')} active={window.location.pathname === '/players'} />
            ...
            <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
                ...
                <main className="flex-1 overflow-y-auto p-6 scrollbar-hide">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
                        {tournaments.filter(t => t.status !== 'completed' && (new Date(t.date).getTime() >= new Date().setHours(0, 0, 0, 0) || t.isLive)).length === 0 ? (
                            <div className="col-span-full text-center text-slate-500 py-10">No upcoming tournaments.</div>
                        ) : (
                            tournaments.filter(t => t.status !== 'completed' && (new Date(t.date).getTime() >= new Date().setHours(0, 0, 0, 0) || t.isLive)).map(tournament => (
